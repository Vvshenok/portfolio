...
async function getClientByDiscordId(discordId) {
  const email = await kvGet('vs:client:discord:' + discordId);
  return email ? getClientByEmail(email) : null;
}

async function linkDiscordClient(client, discordId) {
  client.discordId = discordId;
  await kvSet('vs:client:' + client.email, client);
  await kvSet('vs:client:discord:' + discordId, client.email);
  return client;
}
...
module.exports = {
  ...
  getClientByDiscordId, linkDiscordClient,
